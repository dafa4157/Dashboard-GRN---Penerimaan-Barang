...
# --- Konstanta ---
DATA_FILE = "data.csv"
UPLOAD_PO_DIR = "uploaded_po"
UPLOAD_GRN_DIR = "uploaded_grn"
...
if st.session_state["admin_logged_in"]:
    ...
    filtered_admin_df["Nomor_PO"] = filtered_admin_df["Nomor_PO"].astype(str)
    filtered_admin_df["Nama_Vendor"] = filtered_admin_df["Nama_Vendor"].astype(str)

    if search_po:
        filtered_admin_df = filtered_admin_df[filtered_admin_df["Nomor_PO"].str.contains(search_po, case=False)]
    if search_vendor:
    ...
    if filtered_admin_df.empty:
        ...
        if st.button("Upload File GRN dan Update Status"):
            if not file_grn:
                ...
                os.makedirs(UPLOAD_GRN_DIR, exist_ok=True)
                safe_name = f"{selected_data['Nomor_PO']}_{file_grn.name}"
                grn_path = os.path.join(UPLOAD_GRN_DIR, safe_name).replace("\\", "/")
        ...
        if submitted:
            ...
            if nomor_po == "" or vendor == "":
                ...
                if file_po:
                    ...
                    po_path = os.path.join(UPLOAD_PO_DIR, safe_name).replace("\\", "/")
